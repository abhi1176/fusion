...
#face,palmprint,signature preprocessing
im_size = 224
...
#fusion
merge = Concatenate(axis=1)([out1, out2])  # merge the outputs of the two models
#out = tf.expand_dims(out, axis=1)
fc = RepeatVector(20)(merge)
cnn = Conv1D(32,3,activation='relu',padding='same')(fc)
cnn = MaxPool1D(pool_size=2,strides=2)(cnn)
cnn = Dropout(0.5)(cnn)
cnn = Flatten()(cnn)
cnn = BatchNormalization()(cnn)
last_output = Dense(20, activation='softmax', name='last_output')(cnn)

final_model = Model(inputs=[x1, x2], outputs=[last_output])
final_model.summary()
#opt = SGD(lr=0.01)
#callbacks = keras.callbacks.ModelCheckpoint("save_at_{epoch}.h5")
final_model.compile(loss = "categorical_crossentropy", optimizer = 'adam', metrics=['accuracy'])
final_model.fit({"face_input": face_imgs, "palmprint_input": palm_imgs,"signature_input": signature_imgs,"speaker_input": },Y,epochs=50,
                batch_size=20,shuffle=True,validation_split=0.2)
